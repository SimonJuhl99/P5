...
//   NodeContainer n0n2 = NodeContainer(node.Get(0), node.Get(2));
...
//   NodeContainer n3n5 = NodeContainer(node.Get(3), node.Get(5));
...
//
// Create a BulkSendPtr<ConstantPositionMobilityModel> mn1 = node.Get(1)->GetObject<ConstantPositionMobilityModel> ();
...
//
  ...
  ApplicationContainer sourceApps = source.Install (node.Get (0));
  sourceApps.Add(source2.Install(node.Get(1)));
...
//
  ...
  // ApplicationContainer sinkApps = sink.Install (node.Get (4));



  PacketSinkHelper sink2 ("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));
  sinkApps.Add(sink2.Install(node.Get(5)));
