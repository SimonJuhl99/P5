...
f_same = open("same_dir.data", "r")
f_opposite = open("oppo_dir.data", "r")
...
axs[0,0].plot(same_time, same_values)

axs[0,1].set_title('Opposite satellites')
#axs[0,1].plot(rxtp_time, rxtp_values, color='red')
axs[0,1].plot(opposite_time, opposite_values, color='red')
...
axs[1,1].set_title('BasicSimulator Data Rates for Satellites')

# Create plots with values and define colors and labels
comb_p1, = axs[1,1].plot(same_time, same_values, "b-", label="Parallel sats")
comb_p2, = twin1.plot(opposite_time, opposite_values, "r-", label="Opposite sats")
...
plt.savefig('data_rates_plot.pdf') 
plt.savefig('data_rates_hq.png')  
plt.show()
